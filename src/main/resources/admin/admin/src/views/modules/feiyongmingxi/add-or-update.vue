	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="支出明细" prop="zhichumingxi" >
					<el-input v-model="ruleForm.zhichumingxi" placeholder="支出明细" clearable  :readonly="ro.zhichumingxi"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="支出明细" prop="zhichumingxi" >
					<el-input v-model="ruleForm.zhichumingxi" placeholder="支出明细" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="支出费用" prop="zhichufeiyong" >
					<el-input-number v-model="ruleForm.zhichufeiyong" placeholder="支出费用" :disabled="ro.zhichufeiyong"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="支出费用" prop="zhichufeiyong" >
		...
		data() {
			...
			return {
				...
				rules: {
					...
					zhichumingxi: [
						{ required: true, message: '支出明细不能为空', trigger: 'blur' },
					],
					zhichufeiyong: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `feiyongmingxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.feiyongmingxiCrossAddOrUpdateFlag = false;
											this.parent.search();
