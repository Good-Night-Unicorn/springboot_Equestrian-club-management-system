	...
	export default {
		...
		methods: {
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.huodongxiangqing = this.ruleForm.huodongxiangqing.replace(reg,'../../../springboot1eev7z37/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					if(this.ruleForm.fabushijian>this.ruleForm.huodongshijian){
						this.$message.error("发布时间不能超过活动时间");
						return
					}
