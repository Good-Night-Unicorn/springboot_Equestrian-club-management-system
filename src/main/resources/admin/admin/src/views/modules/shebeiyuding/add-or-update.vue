	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="预订数量" prop="shebeishuliang" >
					<el-input v-model.number="ruleForm.shebeishuliang" placeholder="预订数量" clearable  :readonly="ro.shebeishuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="预订数量" prop="shebeishuliang" >
					<el-input v-model="ruleForm.shebeishuliang" placeholder="预订数量" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="预订人员" prop="yudingrenyuan" >
					<el-input v-model="ruleForm.yudingrenyuan" placeholder="预订人员" clearable  :readonly="ro.yudingrenyuan"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="预订人员" prop="yudingrenyuan" >
					<el-input v-model="ruleForm.yudingrenyuan" placeholder="预订人员" readonly></el-input>
				</el-form-item>
			</template>
	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					...
					yudingbeizhu: [
						{ required: true, message: '预订备注不能为空', trigger: 'blur' },
					],
					yudingrenyuan: [
						{ required: true, message: '预订人员不能为空', trigger: 'blur' },
					],
					userid: [
					],
				},
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'){
					...
					for (var o in obj){
						...
						if(o=='yudingrenyuan'){
							this.ruleForm.yudingrenyuan = obj[o];
							this.ro.yudingrenyuan = true;
							continue;
						}
						if(o=='userid'){
					...
					this.ruleForm.shebeishuliang = 0
					this.ro.shebeishuliang = false;
				}
				// 获取用户信息
				this.$http({
			...
			async onSubmit() {
					...
					if(objcross!=null) {
						if(!this.ruleForm.shebeishuliang){
							this.$message.error("预订数量不能为空");
							return
						}
						objcross.shebeishuliang = objcross.shebeishuliang - this.ruleForm.shebeishuliang
						if(objcross.shebeishuliang<0){
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shebeiyuding/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shebeiyudingCrossAddOrUpdateFlag = false;
											this.parent.search();
