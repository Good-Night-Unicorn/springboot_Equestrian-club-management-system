	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					mafangzhanghao: [
						{ required: true, message: '马房账号不能为空', trigger: 'blur' },
					],
					mafangxingming: [
						{ required: true, message: '马房姓名不能为空', trigger: 'blur' },
					],
					mima: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `mafang/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.mafangCrossAddOrUpdateFlag = false;
											this.parent.search();
