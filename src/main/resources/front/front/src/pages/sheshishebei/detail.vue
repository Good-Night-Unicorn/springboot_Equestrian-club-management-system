	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/sheshishebei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">设备编号</div>
						<div class="text "  >{{detail.shebeibianhao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">设备数量</div>
						<div class="text "  >{{detail.shebeishuliang}}</div>
					</div>
					<div class="item">
						<div class="lable">设备型号</div>
						<div class="text "  >{{detail.shebeixinghao}}</div>
					</div>
					<div class="item">
						<div class="lable">设备用途</div>
						<div class="text "  >{{detail.shebeiyongtu}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('sheshishebei','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('sheshishebei','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('sheshishebei','预订')" 
							@click="onAcross('shebeiyuding','','','','')" type="warning">
							预订
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('sheshishebei','养护')" 
							@click="onAcross('shebeiyanghu','','','','')" type="warning">
							养护
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">设备详情</div>
				</div>
				<div class="desc-text" v-html="detail.shebeixiangqing"></div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`sheshishebei`);
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('sheshishebei/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'sheshishebei', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			backClick(){
				...
				this.$router.push({path: '/index/sheshishebei', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			getDiscussList(page) {
				this.$http.get('discusssheshishebei/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discusssheshishebei/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discusssheshishebei/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discusssheshishebei/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/sheshishebeiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此设施设备？') .then(_ => {
					this.$http.post('sheshishebei/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
