<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="设备数量" prop="shebeishuliang">
				<el-input v-model.number="ruleForm.shebeishuliang" 
					placeholder="设备数量" clearable :disabled=" false  ||ro.shebeishuliang"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="设备用途" prop="shebeiyongtu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="设备用途"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`sheshishebei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`sheshishebei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
