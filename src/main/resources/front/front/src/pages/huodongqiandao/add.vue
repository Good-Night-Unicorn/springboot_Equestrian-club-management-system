<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="活动类型" prop="huodongleixing">
			...
			<el-form-item class="add-item" label="签到人数" prop="huodongrenshu">
				<el-input v-model.number="ruleForm.huodongrenshu" 
					placeholder="签到人数" clearable :disabled=" false  ||ro.huodongrenshu"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="可获积分" prop="jifen">
				<el-input-number v-model="ruleForm.jifen" placeholder="可获积分" :disabled=" false ||ro.jifen"></el-input-number>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`huodongqiandao/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('huodongqiandao/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`huodongqiandao/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
