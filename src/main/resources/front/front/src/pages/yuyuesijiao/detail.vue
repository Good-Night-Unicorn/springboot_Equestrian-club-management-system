	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yuyuesijiao?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yuyuesijiao','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yuyuesijiao','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yuyuesijiao','反馈')" 
							@click="onAcross('pingjiafankui','是','是','','')" type="warning">
							反馈
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('yuyuesijiao','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="payBtn" v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('yuyuesijiao','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`yuyuesijiao`);
			...
			payClick(){
				...
				localStorage.setItem('paytable','yuyuesijiao')
			...
			backClick(){
				...
				this.$router.push({path: '/index/yuyuesijiao', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yuyuesijiaoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此预约私教？') .then(_ => {
					this.$http.post('yuyuesijiao/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
