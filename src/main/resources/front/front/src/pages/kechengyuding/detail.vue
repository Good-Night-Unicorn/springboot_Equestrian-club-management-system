	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/kechengyuding?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">课程难度</div>
						<div class="text "  >{{detail.kechengnandu}}</div>
					</div>
					<div class="item">
						<div class="lable">预订时间</div>
						<div class="text "  >{{detail.yudingshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">会员账号</div>
						<div class="text "  >{{detail.huiyuanzhanghao}}</div>
					</div>
					<div class="item">
						<div class="lable">会员姓名</div>
						<div class="text "  >{{detail.huiyuanxingming}}</div>
					</div>
					<div class="item">
						<div class="lable">预订备注</div>
						<div class="text "  >{{detail.yudingbeizhu}}</div>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('kechengyuding','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('kechengyuding','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('kechengyuding','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="payBtn" v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('kechengyuding','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			payClick(){
				...
				localStorage.setItem('paytable','kechengyuding')
			...
			backClick(){
				...
				this.$router.push({path: '/index/kechengyuding', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/kechengyudingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此课程预订？') .then(_ => {
					this.$http.post('kechengyuding/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
