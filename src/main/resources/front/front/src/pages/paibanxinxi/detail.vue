	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/paibanxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">排班详情</div>
						<div class="text "  >{{detail.paibanxiangqing}}</div>
					</div>
					<div class="item">
						<div class="lable">排班时间</div>
						<div class="text "  >{{detail.paibanshijian}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('paibanxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('paibanxinxi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/paibanxinxi', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/paibanxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此排班信息？') .then(_ => {
					this.$http.post('paibanxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
