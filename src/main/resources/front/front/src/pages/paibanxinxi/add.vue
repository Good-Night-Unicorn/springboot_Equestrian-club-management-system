	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="上班时间" prop="shangbanshijian">
				<el-select v-model="ruleForm.shangbanshijian" placeholder="请选择上班时间" :disabled=" false  ||ro.shangbanshijian" >
					<el-option
						v-for="(item,index) in shangbanshijianOptions"
						:key="index"
						:label="item"
						:value="item">
					</el-option>
			...
			<el-form-item class="add-item" label="排班时间" prop="paibanshijian">
				<el-date-picker
					:disabled=" false  ||ro.paibanshijian"
					format="yyyy 年 MM 月 dd 日"
					value-format="yyyy-MM-dd"
					v-model="ruleForm.paibanshijian" 
					type="date"
					placeholder="排班时间">
				</el-date-picker> 
			</el-form-item>
			<el-form-item class="add-item" label="排班详情" prop="paibanxiangqing">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="排班详情"
					v-model="ruleForm.paibanxiangqing">
					</el-input>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.paibanshijian = this.getCurDate()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='paibanxiangqing'){
							this.ruleForm.paibanxiangqing = obj[o];
							this.ro.paibanxiangqing = true;
							continue;
						}
						if(o=='tupian'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`paibanxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`paibanxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
