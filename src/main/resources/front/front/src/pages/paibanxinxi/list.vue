	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('paibanxinxi','新增')" type="primary" @click="add('/index/paibanxinxiAdd')">
...
<script>
	export default {
		...
		async created() {
			...
			this.selectOptionsList.push({name:'上班时间',list:this.shangbanshijianOptions,tableName: 'shangbanshijian',check: -1})
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.shangbanshijian != '') searchWhere.shangbanshijian = this.formSearch.shangbanshijian;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`paibanxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/paibanxinxiDetail', query: params});
			},
