<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="预订数量" prop="shebeishuliang">
				<el-input v-model.number="ruleForm.shebeishuliang" 
					placeholder="预订数量" clearable :disabled=" false  ||ro.shebeishuliang"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="预订人员" prop="yudingrenyuan">
				<el-input v-model="ruleForm.yudingrenyuan" 
					placeholder="预订人员" clearable :disabled=" false  ||ro.yudingrenyuan"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`shebeiyuding/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				obj.shebeishuliang = obj.shebeishuliang - this.ruleForm.shebeishuliang
				if(obj.shebeishuliang<0){
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						obj.shebeishuliang = parseFloat(obj.shebeishuliang) - parseFloat(this.ruleForm.shebeishuliang)

						await this.$http.post(table+`/update`,obj).then(res => {});
						await this.$http.post(`shebeiyuding/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
