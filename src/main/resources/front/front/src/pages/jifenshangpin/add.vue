<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="库存数量" prop="shangpinshuliang">
				<el-input v-model.number="ruleForm.shangpinshuliang" 
					placeholder="库存数量" clearable :disabled=" false  ||ro.shangpinshuliang"></el-input>
			</el-form-item>
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jifenshangpin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
