<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="训练结果" prop="xunlianjieguo">
				<el-select v-model="ruleForm.xunlianjieguo" placeholder="请选择训练结果" :disabled=" false  ||ro.xunlianjieguo" >
					<el-option
						v-for="(item,index) in xunlianjieguoOptions"
						:key="index"
						:label="item"
						:value="item">
					</el-option>
			...
			<el-form-item class="add-item" label="训练详情" prop="xunlianxiangqing">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="训练详情"
					v-model="ruleForm.xunlianxiangqing">
					</el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='xunlianjieguo'){
							this.ruleForm.xunlianjieguo = obj[o];
							this.ro.xunlianjieguo = true;
							continue;
						}
						if(o=='xunlianxiangqing'){
							this.ruleForm.xunlianxiangqing = obj[o];
							this.ro.xunlianxiangqing = true;
							continue;
						}
					}
				}else if(type=='edit'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`mapixunlian/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`mapixunlian/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
