		...
		<el-form
			...
			<el-form-item class="add-item" label="马匹种类" prop="mapizhonglei">
				<el-input v-model="ruleForm.mapizhonglei" 
					placeholder="马匹种类" clearable :disabled=" false  ||ro.mapizhonglei"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="马匹性别" prop="mapixingbie">
				<el-input v-model="ruleForm.mapixingbie" 
					placeholder="马匹性别" clearable :disabled=" false  ||ro.mapixingbie"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item"  label="健康类型" prop="jiankangleixing">
				<el-select v-model="ruleForm.jiankangleixing" placeholder="请选择健康类型" :disabled=" false  ||ro.jiankangleixing" >
					<el-option
			...
			<el-form-item class="add-item" label="记录详情" prop="jiluxiangqing">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="记录详情"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`mapijiankang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`mapijiankang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
