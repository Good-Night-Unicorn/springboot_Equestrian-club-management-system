	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/changdileixing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('changdileixing','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('changdileixing','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此场地类型？') .then(_ => {
