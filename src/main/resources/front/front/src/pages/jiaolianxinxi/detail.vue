	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiaolianxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">教练账号</div>
						<div class="text "  >{{detail.jiaolianzhanghao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">私教费用</div>
						<div class="text "  >{{detail.sijiaofeiyong}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">资质证书</div>
						<div class="text "  >{{detail.zizhizhengshu}}</div>
					</div>
					<div class="item">
						<div class="lable">教学经历</div>
						<div class="text "  >{{detail.jiaoxuejingli}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiaolianxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiaolianxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('jiaolianxinxi','预约')" 
							@click="onAcross('yuyuesijiao','','','','')" type="warning">
							预约
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				...
				<div class="desc-text" v-html="detail.gerenjianjie"></div>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('jiaolianxinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'jiaolianxinxi', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			backClick(){
				...
				this.$router.push({path: '/index/jiaolianxinxi', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			getDiscussList(page) {
				this.$http.get('discussjiaolianxinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussjiaolianxinxi/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussjiaolianxinxi/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussjiaolianxinxi/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jiaolianxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此教练信息？') .then(_ => {
					this.$http.post('jiaolianxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
