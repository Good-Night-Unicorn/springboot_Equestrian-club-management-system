	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/changdiyuding?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">预订时段</div>
						<div class="text "  >{{detail.keyueshiduan}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('changdiyuding','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('changdiyuding','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/changdiyuding', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此场地预订？') .then(_ => {
					this.$http.post('changdiyuding/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
