<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="预订时段" prop="keyueshiduan">
				<el-input v-model="ruleForm.keyueshiduan" 
					placeholder="预订时段" clearable :disabled=" false  ||ro.keyueshiduan"></el-input>
			</el-form-item>
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`changdiyuding/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
