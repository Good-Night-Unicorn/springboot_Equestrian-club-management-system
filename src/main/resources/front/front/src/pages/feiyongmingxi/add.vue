<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="支出明细" prop="zhichumingxi">
				<el-input v-model="ruleForm.zhichumingxi" 
					placeholder="支出明细" clearable :disabled=" false  ||ro.zhichumingxi"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="支出费用" prop="zhichufeiyong">
				<el-input-number v-model="ruleForm.zhichufeiyong" placeholder="支出费用" :disabled=" false ||ro.zhichufeiyong"></el-input-number>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`feiyongmingxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`feiyongmingxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
